		...
		<Dialog open={open} onClose={() => onOpenChange(false)} className="relative z-50">
			...
			<div className="fixed inset-0 z-10 w-screen overflow-y-auto">
				<div className="flex min-h-full justify-center items-center">
					<DialogPanel
						className={`${clasName} !size-full relative transform rounded-lg bg-card transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in data-closed:sm:translate-y-0 data-closed:sm:scale-95`}
						children={children}
						transition
					/>
				</div>
